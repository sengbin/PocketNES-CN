...
,0x43,0xb3,0xe1,0x21,0x61,0x41,0x81,0x10,0x62,0x53,0x80,0x73,0x95,0x6e,0xa0,0xd9,0x3e,0xe1,0x95,0x0d,0x10,0xdf,0x2b,0xf3,0x20,0xec,0x00};

APPEND void init_speed_hacks()
{
	depack(quickhackfinder_ins_table_ap, quickhackfinder_ins_table);
}
...
out_loop:
	...
	}
	{
		...
		hack->divider=GetReciprocal(total_cycles)+1;
	}
...
bool game_specific_hack(const u8 *initpc, const u8 *lastbank, int hacknum)
{
	...
	else
	{
		...
		if (konami_hack_number>=0)
		{
			...
			sh->divider = konami_speedhack_cycles_inverse[konami_hack_number];
...
static __inline u32 GetReciprocal(u32 divisor)
{
	//dividend = 0x1 0000 0000
	//divisor = i

	if (divisor <= 1) return 0xFFFFFFFF;
	u32 leadingZeroes = CountLeadingZeroes(divisor);
	u32 mask = 1 << leadingZeroes;
	divisor = divisor << leadingZeroes;
	if (divisor == 0x80000000) return mask << 1;
